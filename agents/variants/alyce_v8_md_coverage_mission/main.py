...
class ProducerLiteConfig:
    ...
    # V8 selected-action safety filter. Unlike the rejected V4 branch, this
    # starts from V2 and only changes the current top candidate when a clearly
    # safer close-score alternative exists. V8 keeps the V6 official-best base,
    # adds per-candidate trace labels, and uses replay-derived risk only in 4P.
    ...
    enable_multi_size_drain: bool = False
    multi_size_drain_tiers: tuple[float, ...] = (1.0,)
    enable_v8_md_coverage_filter: bool = False
    enable_v8_candidate_trace: bool = False
    v8_recovery_step_start: int = 35
    v8_recovery_step_end: int = 190
    v8_soft_prod_gap_trigger: float = -3.0
    v8_soft_prod_rank_trigger: int = 2
    v8_risk_threshold: float = 2.15
    v8_alt_score_gap: float = 6.0
    v8_bad_reaction_gap: float = 2.0
    v8_safe_reaction_gap: float = 0.0
    v8_frontier_dist: float = 48.0
    v8_depletion_ratio: float = 0.72
    v8_high_source_prod_min: float = 3.0
    v8_low_target_prod_max: float = 3.0
    v8_leader_low_value_prod_max: float = 2.0
    v8_force_bonus: float = 0.10
...
def _apply_v5_selected_action_filter(
    *,
    ...
    prod_leader = int(prod_by_owner.argmax().detach().item()) if prod_by_owner.numel() > 0 else pid
    leader_prod_t = prod_by_owner[prod_leader] if 0 <= prod_leader < int(player_count) else torch.tensor(0.0, dtype=dtype, device=device)
    ...
    v8_recovery_active = (
        bool(config.enable_v8_md_coverage_filter)
        and int(config.v8_recovery_step_start) <= int(step) <= int(config.v8_recovery_step_end)
        and (
            prod_gap <= float(config.v8_soft_prod_gap_trigger)
            or prod_rank >= int(config.v8_soft_prod_rank_trigger)
        )
    )
    ...
    source_critical = (source_prod >= float(config.v8_high_source_prod_min)) | (
        pressure_s >= source_ships * float(config.ffa_source_pressure_gate)
    )
    low_impact_leader_target = (
        owner_is_enemy
        & (owner == leader)
        & (target_prod <= float(config.v8_leader_low_value_prod_max))
        & (target_distance >= float(config.v8_frontier_dist) * 0.75)
        & (reaction_gap <= float(config.v8_bad_reaction_gap))
    )
    ...
    v8_risk = torch.zeros_like(score)
    if v8_recovery_active:
        v8_risk = (
            torch.where(~owner_is_mine & source_critical, torch.full_like(score, 0.60), torch.zeros_like(score))
            + torch.where(depletion_ratio >= float(config.v8_depletion_ratio), torch.full_like(score, 0.70), torch.zeros_like(score))
            + torch.where(target_prod <= float(config.v8_low_target_prod_max), torch.full_like(score, 0.45), torch.zeros_like(score))
            + torch.where(reaction_gap <= float(config.v8_bad_reaction_gap), torch.full_like(score, 0.55), torch.zeros_like(score))
            + torch.where(target_distance >= float(config.v8_frontier_dist), torch.full_like(score, 0.35), torch.zeros_like(score))
            + torch.where(low_impact_leader_target, torch.full_like(score, 0.65), torch.zeros_like(score))
            + torch.full_like(score, max(0.0, -prod_gap) * 0.08 + max(0, prod_rank - 1) * 0.25)
        )
    v8_replay_risky = v8_recovery_active & (v8_risk >= float(config.v8_risk_threshold))
    bad_top_mask = bad_top_mask | gap_mode_bad_attack | v8_replay_risky
    bad_top = bool(fired and bad_top_mask[before_idx].detach().item())

    before_reason = "ok"
    if bad_top:
        if bool(v8_replay_risky[before_idx].detach().item()):
            before_reason = "v8_replay_risk_with_safe_alt"
        elif bool(gap_mode_bad_attack[before_idx].detach().item()):
    ...
    alt_score_gap = (
        float(config.v8_alt_score_gap)
        if v8_recovery_active
        else (float(config.v6_gap_alt_score_gap) if v6_gap_active else float(config.v5_alt_score_gap))
    )
    ...
    v8_alt_ok = (
        ...
        & (
            owner_is_mine
            | (target_prod >= target_prod[before_idx] + 1.0)
            | (target_prod >= 3.0)
            | (reaction_gap >= float(config.v8_safe_reaction_gap))
            | (target_distance <= float(config.v8_frontier_dist))
            | (depletion_ratio <= depletion_ratio[before_idx] - 0.06)
        )
    )
    ...
    not_bad_alt = ~(low_far_neutral | low_far_nonleader_enemy | depleted_source | gap_mode_bad_attack | v8_replay_risky)
    ...
    alt_mask = (
        ...
        & score_gap_ok
        & not_bad_alt
        & (prod_gain_ok | reaction_ok | depletion_ok | gap_reaction_ok | gap_regroup_ok | gap_frontier_ok | v8_alt_ok)
    )
    ...
    if bool(alt_mask.any()):
        ...
        if v8_recovery_active:
            gap_alt_bonus = gap_alt_bonus + (
                torch.where(owner_is_mine, torch.full_like(score, 2.15), torch.zeros_like(score))
                + target_prod.clamp(min=0.0, max=5.0) * 0.55
                + reaction_gap.clamp(min=-2.0, max=8.0) * 0.24
                + torch.where(target_distance <= float(config.v8_frontier_dist), torch.full_like(score, 0.55), torch.zeros_like(score))
                - target_distance.clamp(min=0.0, max=120.0) * 0.010
                - depletion_ratio.clamp(min=0.0, max=1.0) * 0.70
            )
        ...
        adjusted[after_idx] = torch.maximum(
            adjusted[after_idx],
            before_score + (
                float(config.v8_force_bonus)
                if v8_recovery_active
                else (float(config.v6_gap_force_bonus) if v6_gap_active else float(config.v5_force_bonus))
            ),
        )
    ...
    if bool(config.enable_v5_trace):
        ...
        top_candidates = []
        if bool(config.enable_v8_candidate_trace):
            trace_scores = torch.where(valid, score, torch.full_like(score, float("-inf")))
            top_n = min(5, int(trace_scores.numel()))
            if top_n > 0:
                top_idx = torch.argsort(trace_scores, descending=True, stable=True)[:top_n]
                for raw_idx in top_idx.detach().cpu().tolist():
                    if not bool(torch.isfinite(trace_scores[raw_idx]).detach().item()):
                        continue
                    labels = []
                    if bool(owner_is_mine[raw_idx].detach().item()):
                        labels.append("mine_or_regroup")
                    elif bool(owner_is_neutral[raw_idx].detach().item()):
                        labels.append("neutral")
                    elif bool(owner_is_enemy[raw_idx].detach().item()):
                        labels.append("enemy")
                    if bool(low_far_neutral[raw_idx].detach().item()):
                        labels.append("low_far_neutral")
                    if bool(low_far_nonleader_enemy[raw_idx].detach().item()):
                        labels.append("low_far_nonleader_enemy")
                    if bool(depleted_source[raw_idx].detach().item()):
                        labels.append("depleted_source")
                    if bool(gap_mode_bad_attack[raw_idx].detach().item()):
                        labels.append("v6_gap_bad_attack")
                    if bool(v8_replay_risky[raw_idx].detach().item()):
                        labels.append("v8_replay_risky")
                    if bool(low_impact_leader_target[raw_idx].detach().item()):
                        labels.append("low_impact_leader")
                    top_candidates.append({
                        "idx": int(raw_idx),
                        "score": item_float(score, raw_idx),
                        "src": item_int(flat_src, raw_idx),
                        "tgt": item_int(flat_tgt, raw_idx),
                        "send": item_float(flat_send, raw_idx),
                        "owner": item_int(owner, raw_idx),
                        "target_prod": item_float(target_prod, raw_idx),
                        "source_prod": item_float(source_prod, raw_idx),
                        "distance": item_float(target_distance, raw_idx),
                        "reaction_gap": item_float(reaction_gap, raw_idx),
                        "depletion_ratio": item_float(depletion_ratio, raw_idx),
                        "v8_risk": item_float(v8_risk, raw_idx),
                        "labels": labels,
                    })
        ...
        _V6_TRACE_LAST = {
            ...
            "prod_leader": int(prod_leader),
            "strength_leader": int(leader),
            "v6_gap_active": bool(v6_gap_active),
            "v8_recovery_active": bool(v8_recovery_active),
            "v8_before_risk": item_float(v8_risk, before_idx),
            ...
            "v8_replay_risky_count": int((v8_replay_risky & valid).sum().detach().item()),
            "low_impact_leader_count": int((low_impact_leader_target & valid).sum().detach().item()),
...
def plan_lite_waves(
    *,
    ...
    # V8 can expose several send-size tiers to the existing greedy selector.
    # This directly addresses the replay pattern where one full-source launch
    # wins a local fight but leaves a high-value frontier source empty.
    if bool(config.enable_multi_size_drain) and int(player_count) >= 4:
        tier_values = tuple(float(x) for x in config.multi_size_drain_tiers if float(x) > 0.0)
        if not tier_values:
            tier_values = (1.0,)
        tier = torch.tensor(tier_values, dtype=dtype, device=device).clamp(min=0.10, max=1.0)
    else:
        tier = torch.ones(1, dtype=dtype, device=device)
    G = int(tier.numel())
    sizes = (
        drain.view(S, 1, 1).expand(S, T, G)
        * tier.view(1, 1, G)
    ).floor().clamp(min=1.0)
    ...
    if K > 0:
        ...
        floor_at_arr = floor_expanded.gather(-1, k_arr.unsqueeze(-1)).squeeze(-1)
    else:
    ...
    valid = (
        ...
        & src_neq_tgt & source_exists.view(S, 1, 1) & target_exists.view(1, T, 1)
    )

    L = 1
    C = S * T * G
    source_grid = source_idx.view(S, 1, 1).expand(S, T, G)
    target_grid = target_idx.view(1, T, 1).expand(S, T, G)
    target_short_grid = torch.arange(T, device=device).view(1, T, 1).expand(S, T, G)
    target_is_mine_grid = target_is_mine.view(1, T, 1).expand(S, T, G)
    cand_src = source_grid.reshape(C, L)
    cand_tgt_slot = target_grid.reshape(C)
    cand_tgt_short = target_short_grid.reshape(C)
    ...
    cand_is_def = target_is_mine_grid.reshape(C)
...
CONFIG_4P = replace(
    ...
    enable_multi_size_drain=True,
    multi_size_drain_tiers=(0.60, 0.80, 1.0),
    enable_v8_md_coverage_filter=True,
    enable_v8_candidate_trace=True,
)
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        trace_path = (
            os.environ.get("ORBIT_V8_TRACE_PATH")
            or os.environ.get("ORBIT_V6_TRACE_PATH")
            or os.environ.get("ORBIT_V5_TRACE_PATH")
        )
        if trace_path and _V6_TRACE_LAST:
            try:
                trace_dir = os.path.dirname(trace_path)
                if trace_dir:
                    os.makedirs(trace_dir, exist_ok=True)
                with open(trace_path, "a", encoding="utf-8") as handle:
                    handle.write(json.dumps(_V6_TRACE_LAST, sort_keys=True) + "\n")
            except Exception:
                pass
