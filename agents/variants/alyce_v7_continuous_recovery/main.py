...
class ProducerLiteConfig:
    ...
    # V7 selected-action safety filter. Unlike the rejected V4 branch, this
    # starts from V2 and only changes the current top candidate when a clearly
    # safer close-score alternative exists. V7 keeps V6's hard recovery gate but
    # adds a continuous recovery risk score based on the real production leader.
    ...
    enable_v7_continuous_recovery: bool = False
    v7_recovery_step_start: int = 35
    v7_recovery_step_end: int = 190
    v7_soft_prod_gap_trigger: float = -3.0
    v7_soft_prod_rank_trigger: int = 2
    v7_risk_threshold: float = 2.25
    v7_alt_score_gap: float = 6.5
    v7_bad_reaction_gap: float = 2.0
    v7_safe_reaction_gap: float = 0.0
    v7_frontier_dist: float = 48.0
    v7_depletion_ratio: float = 0.72
    v7_high_source_prod_min: float = 3.0
    v7_low_target_prod_max: float = 3.0
    v7_leader_low_value_prod_max: float = 2.0
    v7_force_bonus: float = 0.10
...
def _apply_v5_selected_action_filter(
    *,
    ...
    v7_recovery_active = (
        bool(config.enable_v7_continuous_recovery)
        and int(config.v7_recovery_step_start) <= int(step) <= int(config.v7_recovery_step_end)
        and (
            prod_gap <= float(config.v7_soft_prod_gap_trigger)
            or prod_rank >= int(config.v7_soft_prod_rank_trigger)
        )
    )
    ...
    low_impact_leader_target = (
        owner_is_enemy
        & (owner == leader)
        & (target_prod <= float(config.v7_leader_low_value_prod_max))
        & (target_distance >= float(config.v7_frontier_dist) * 0.75)
        & (reaction_gap <= float(config.v7_bad_reaction_gap))
    )
    ...
    v7_recovery_risk = torch.zeros_like(score)
    if v7_recovery_active:
        important_source = (source_prod >= float(config.v7_high_source_prod_min)) | (
            pressure_s >= source_ships * float(config.ffa_source_pressure_gate)
        )
        v7_recovery_risk = (
            torch.where(~owner_is_mine & important_source, torch.full_like(score, 0.60), torch.zeros_like(score))
            + torch.where(depletion_ratio >= float(config.v7_depletion_ratio), torch.full_like(score, 0.70), torch.zeros_like(score))
            + torch.where(target_prod <= float(config.v7_low_target_prod_max), torch.full_like(score, 0.45), torch.zeros_like(score))
            + torch.where(reaction_gap <= float(config.v7_bad_reaction_gap), torch.full_like(score, 0.55), torch.zeros_like(score))
            + torch.where(target_distance >= float(config.v7_frontier_dist), torch.full_like(score, 0.35), torch.zeros_like(score))
    ...
    v7_risky_candidate = v7_recovery_active & (v7_recovery_risk >= float(config.v7_risk_threshold))
    bad_top_mask = bad_top_mask | gap_mode_bad_attack | v7_risky_candidate
    bad_top = bool(fired and bad_top_mask[before_idx].detach().item())

    before_reason = "ok"
    if bad_top:
        if bool(v7_risky_candidate[before_idx].detach().item()):
            before_reason = "v7_recovery_risky_top_with_safe_alt"
    ...
    v7_recovery_alt_ok = (
        ...
        & (
            ...
            | (target_prod >= 3.0)
            | (reaction_gap >= float(config.v7_safe_reaction_gap))
            | (target_distance <= float(config.v7_frontier_dist))
    ...
    not_bad_alt = ~(low_far_neutral | low_far_nonleader_enemy | depleted_source | gap_mode_bad_attack | v7_risky_candidate)
    ...
    alt_mask = (
        ...
        & score_gap_ok
        & not_bad_alt
        & (prod_gain_ok | reaction_ok | depletion_ok | gap_reaction_ok | gap_regroup_ok | gap_frontier_ok | v7_recovery_alt_ok)
    )
    ...
    if bool(alt_mask.any()):
        ...
        if v7_recovery_active:
            gap_alt_bonus = gap_alt_bonus + (
                ...
                + torch.where(target_distance <= float(config.v7_frontier_dist), torch.full_like(score, 0.55), torch.zeros_like(score))
    ...
    if bool(config.enable_v5_trace):
        ...
        _V6_TRACE_LAST = {
            ...
            "v7_recovery_active": bool(v7_recovery_active),
            "v7_before_risk": item_float(v7_recovery_risk, before_idx),
            ...
            "v7_risky_candidate_count": int((v7_risky_candidate & valid).sum().detach().item()),
        }
    return adjusted
...
CONFIG_4P = replace(
    ...
    enable_v7_continuous_recovery=True,
)
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        trace_path = (
            os.environ.get("ORBIT_V7_TRACE_PATH")
