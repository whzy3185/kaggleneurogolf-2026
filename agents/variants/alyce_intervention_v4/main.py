...
class ProducerLiteConfig:
    ...
    # V4 4P context scoring. This remains a soft scorer, not a launch veto.
    # The replay reviews showed that static far-low penalties are insufficient:
    # V4 adds rank/phase context, rough reaction distance, source protection,
    # and a narrower leader exception.
    enable_ffa_v4_safety: bool = False
    enable_ffa_v4_trace: bool = False
    v4_far_enemy_dist: float = 50.0
    v4_far_enemy_prod_max: float = 2.0
    v4_far_enemy_penalty: float = 1.35
    v4_far_enemy_dist_penalty: float = 0.02
    v4_low_value_neutral_dist: float = 55.0
    v4_low_value_neutral_prod_max: float = 1.0
    v4_low_value_neutral_penalty: float = 1.15
    v4_trap_neutral_penalty: float = 2.0
    v4_reaction_gap_dist: float = 18.0
    v4_hold_margin_ships: float = 8.0
    v4_safe_neutral_bonus: float = 0.45
    v4_source_protect_penalty: float = 0.45
    v4_high_prod_source_min: float = 4.0
    v4_source_after_floor: float = 4.0
    v4_trailing_enemy_pressure_penalty: float = 1.25
    v4_leader_low_value_hold_penalty: float = 1.20
...
CONFIG_4P = dataclasses.replace(
    ...
    enable_ffa_v4_safety=True,
    enable_ffa_v4_trace=True,
)
...
def _prod_by_owner(obs, prod, player_count, dtype, device):
    owner = obs.owner_abs.to(torch.long)
    owner_valid = (owner >= 0) & (owner < int(player_count)) & obs.alive
    owner_idx = owner.clamp(min=0, max=max(int(player_count) - 1, 0))
    prod_by_owner = torch.zeros(int(player_count), dtype=dtype, device=device)
    ...
    return prod_by_owner


def _apply_ffa_v4_context_score(
    score: Tensor,
    *,
    obs,
    cache,
    ...
    pid: int,
    step: int,
    enemy_mass: Tensor | None = None,
    dtype,
    device,
) -> Tensor:
    """4P context scorer derived from V2/V3 official replay failures.

    V3 only demoted far low-value targets. The official replay review showed
    broader failures: leader-owned low-value exceptions, non-holdable captures,
    early production-rank collapse, attack-over-regroup behavior, and source
    depletion from high-production frontier planets. This scorer keeps the
    Intervention planner's candidate set intact and changes only the score.
    """
    if not bool(config.enable_ffa_v4_safety) or int(player_count) < 4:
        return score
    ...
    flat_send = cand_send.reshape(-1).to(dtype)
    flat_eta = cand_eta.reshape(-1).to(dtype)
    ...
    target_is_neutral = obs.is_neutral[target_planets]

    strength = _compute_strength(obs, prod, int(player_count), dtype, device)
    prod_strength = _prod_by_owner(obs, prod, int(player_count), dtype, device)
    leader = int(torch.argmax(strength).detach().item()) if int(player_count) > 0 else int(pid)
    my_prod = prod_strength[int(pid)]
    prod_rank = int((prod_strength > my_prod).sum().detach().item()) + 1
    leader_prod = prod_strength[leader] if int(player_count) > 0 else my_prod
    ...
    enemy_planets = (
        ...
        & (owner_abs >= 0)
        & (owner_abs != int(pid))
        & (owner_abs < int(player_count))
    )
    if bool(enemy_planets.any()):
        enemy_idx = torch.nonzero(enemy_planets, as_tuple=False).reshape(-1)
        enemy_best_dist = cache.cross_dist[0].to(dtype)[flat_tgt][:, enemy_idx].amin(dim=1)
    else:
        enemy_best_dist = torch.full_like(immediate_dist, float("inf"))
    reaction_bad = enemy_best_dist <= (immediate_dist + float(config.v4_reaction_gap_dist))

    target_current_ships = obs.ships[flat_tgt].to(dtype)
    post_capture_margin = flat_send - target_current_ships
    hold_weak = post_capture_margin <= float(config.v4_hold_margin_ships)

    valid_flat = cand_valid.reshape(-1)
    cand_def_flat = cand_is_def.reshape(-1)
    target_prod_flat = target_prod[flat_tgt_short].to(dtype)
    enemy_low_value = (
        target_is_enemy[flat_tgt_short]
        & (target_prod_flat <= float(config.v4_far_enemy_prod_max))
        & ~cand_def_flat
    )
    enemy_far = immediate_dist >= float(config.v4_far_enemy_dist)
    enemy_dist_penalty = (
        float(config.v4_far_enemy_penalty)
        + (immediate_dist - float(config.v4_far_enemy_dist)).clamp(min=0.0)
        * float(config.v4_far_enemy_dist_penalty)
    )

    neutral_low_value = (
        target_is_neutral[flat_tgt_short]
        & (target_prod_flat <= float(config.v4_low_value_neutral_prod_max))
        & ~cand_def_flat
    )
    neutral_far = immediate_dist >= float(config.v4_low_value_neutral_dist)

    neutral_target = target_is_neutral[flat_tgt_short] & ~cand_def_flat
    trap_neutral = neutral_target & reaction_bad & hold_weak
    safe_neutral = neutral_target & ~reaction_bad & (target_prod_flat >= 3.0) & (immediate_dist <= 70.0)

    source_ships = obs.ships[flat_src].to(dtype)
    source_after = (source_ships - flat_send).clamp(min=0.0)
    source_prod = prod[flat_src].to(dtype)
    source_pressure = (
        enemy_mass[flat_src].to(dtype)
        if enemy_mass is not None
        else torch.zeros_like(source_after)
    )
    low_context_target = neutral_low_value | enemy_low_value | reaction_bad
    source_protect = (
        (source_prod >= float(config.v4_high_prod_source_min))
        & (source_after < float(config.v4_source_after_floor))
        & (source_pressure > 0.75)
        & low_context_target
        & ~cand_def_flat
    )
    ...
    trailing_early_mid = (
        (int(step) >= 45)
        & (int(step) <= 170)
        & ((prod_rank >= 3) | (float(prod_gap.detach().item()) <= -6.0))
    )
    leading_opening = (
        (int(step) <= 120)
        & (prod_rank == 1)
        & (float(prod_gap.detach().item()) >= -1.0)
    )
    non_leader_enemy = enemy_low_value & ~target_is_leader[flat_tgt_short]
    leader_low_value_weak = (
        enemy_low_value
        & target_is_leader[flat_tgt_short]
        & (reaction_bad | hold_weak | source_protect)
    )
    trailing_bad_attack = (
        bool(trailing_early_mid)
        & non_leader_enemy
        & (enemy_far | reaction_bad | hold_weak)
    )
    lead_bleed = (
        bool(leading_opening)
        & (neutral_low_value | non_leader_enemy)
        & (neutral_far | enemy_far | reaction_bad)
    )

    penalty = torch.zeros_like(score)
    penalty = penalty + torch.where(
        non_leader_enemy & enemy_far & valid_flat,
        enemy_dist_penalty,
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        neutral_low_value & neutral_far & valid_flat,
        torch.full_like(score, float(config.v4_low_value_neutral_penalty)),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        trap_neutral & valid_flat,
        torch.full_like(score, float(config.v4_trap_neutral_penalty)),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        source_protect & valid_flat,
        torch.full_like(score, float(config.v4_source_protect_penalty)),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        trailing_bad_attack & valid_flat,
        torch.full_like(score, float(config.v4_trailing_enemy_pressure_penalty)),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        lead_bleed & valid_flat,
        torch.full_like(score, float(config.v4_low_value_neutral_penalty)),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        leader_low_value_weak & valid_flat,
        torch.full_like(score, float(config.v4_leader_low_value_hold_penalty)),
        torch.zeros_like(score),
    )
    bonus = torch.where(
        safe_neutral & valid_flat,
        torch.full_like(score, float(config.v4_safe_neutral_bonus)),
        torch.zeros_like(score),
    )
    adjusted = score + bonus - penalty

    if bool(config.enable_ffa_v4_trace):
        global _V4_TRACE_LAST
        masked_before = torch.where(valid_flat & torch.isfinite(score), score, torch.full_like(score, float("-inf")))
        masked_after = torch.where(valid_flat & torch.isfinite(adjusted), adjusted, torch.full_like(adjusted, float("-inf")))
        before_idx = int(torch.argmax(masked_before).detach().item()) if int(masked_before.numel()) else -1
        after_idx = int(torch.argmax(masked_after).detach().item()) if int(masked_after.numel()) else -1
        _V4_TRACE_LAST = {
            "step": int(step),
            "player_count": int(player_count),
            "prod_rank": int(prod_rank),
            "prod_gap": float(prod_gap.detach().item()),
            "leader": int(leader),
            "top_changed": bool(before_idx != after_idx),
            "trap_neutral": int((trap_neutral & valid_flat).sum().detach().item()),
            "safe_neutral_bonus": int((safe_neutral & valid_flat).sum().detach().item()),
            "far_low_nonleader_enemy": int((non_leader_enemy & enemy_far & valid_flat).sum().detach().item()),
            "leader_low_value_weak": int((leader_low_value_weak & valid_flat).sum().detach().item()),
            "source_protect": int((source_protect & valid_flat).sum().detach().item()),
            "trailing_bad_attack": int((trailing_bad_attack & valid_flat).sum().detach().item()),
        }
    return adjusted
...
def plan_lite_waves(
    *,
    ...
    score = _apply_ffa_v4_context_score(
        score,
        obs=obs,
        cache=cache,
        prod=prod,
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict, comet_info: dict | None = None):
        ...
        trace_path = os.environ.get("ORBIT_V4_TRACE_PATH")
        if trace_path and _V4_TRACE_LAST:
            try:
                with open(trace_path, "a", encoding="utf-8") as f:
                    f.write(json.dumps(_V4_TRACE_LAST, sort_keys=True) + "\n")
            except Exception:
                pass
