...
class ProducerLiteConfig:
    ...
    # V9 mission router. This is pre-greedy 4P candidate shaping, not another
    # selected-action recovery threshold. It targets the public-sacrifice pattern
    # seen in V6/V7/V8 official replays: early center/shared-zone actions that
    # can be cleaned up or answered by multiple opponents.
    enable_v9_mission_router: bool = False
    v9_opening_end: int = 35
    v9_center_step_end: int = 90
    v9_center_dist: float = 30.0
    v9_far_dist: float = 55.0
    v9_safe_reaction_gap: float = 4.0
    v9_close_enemy_eta_margin: float = 8.0
    v9_public_depletion_ratio: float = 0.72
    v9_opening_public_penalty: float = 12.0
    v9_center_public_penalty: float = 9.0
    v9_public_hard_reaction_gap: float = -4.0
    v9_public_low_prod_max: float = 2.0
    v9_trailing_prod_gap: float = -6.0
    v9_trailing_prod_rank: int = 3
    v9_trailing_low_enemy_penalty: float = 10.0
    v9_leader_asset_prod_min: float = 3.0
    v9_leader_hold_reaction_gap: float = -0.5
...
def _apply_4p_candidate_adjustments(
    *,
    ...
    if bool(config.enable_v9_mission_router):
        ...
        if per_enemy_eta:
            ...
            cand_enemy_eta = enemy_eta_by_owner[:, cand_tgt_short]
            close_enemy_count = (
                cand_enemy_eta <= (eta.unsqueeze(0) + float(config.v9_close_enemy_eta_margin))
    ...
    distance_now = cache.cross_dist[0][src, tgt].to(dtype=dtype, device=device)
    target_center_dist = torch.sqrt((obs.x.to(dtype=dtype, device=device)[tgt] - 50.0) ** 2 + (obs.y.to(dtype=dtype, device=device)[tgt] - 50.0) ** 2)
    ...
    if bool(config.enable_v9_mission_router):
        ...
        owner_all = obs.owner_abs.to(device=device)
        prod_all = prod.to(dtype=dtype, device=device)
        alive_all = obs.alive.to(device=device)
        for oid in range(int(player_count)):
            mask = alive_all & (owner_all == float(oid))
            if bool(mask.any()):
                prod_by_owner[oid] = prod_all[mask].sum()
        ...
        leader_prod = prod_by_owner[prod_leader] if 0 <= prod_leader < int(player_count) else my_prod
        prod_gap = my_prod - leader_prod
        prod_rank = int((prod_by_owner > my_prod).sum().detach().item()) + 1
        trailing = (prod_gap <= float(config.v9_trailing_prod_gap)) | (prod_rank >= int(config.v9_trailing_prod_rank))

        center_or_middle = target_center_dist <= float(config.v9_center_dist)
        far_target = distance_now >= float(config.v9_far_dist)
        shared_zone = close_enemy_count >= 2.0
        unsafe_reaction = reaction_gap < float(config.v9_safe_reaction_gap)
        public_target = is_neutral | is_enemy_target
        source_public_depletion = depletion_ratio >= float(config.v9_public_depletion_ratio)

        opening_public_sacrifice = (
            (int(step) < int(config.v9_opening_end))
            & public_target
            & (center_or_middle | far_target | shared_zone)
            & unsafe_reaction
            & (source_public_depletion | (prod_s >= 3.0))
            & cand_valid
        )
        public_sacrifice_score = (
            float(config.v9_opening_public_penalty)
            + torch.where(center_or_middle, torch.full_like(score, 4.0), torch.zeros_like(score))
            + torch.where(shared_zone, close_enemy_count * 1.5, torch.zeros_like(score))
            + torch.where(far_target, (distance_now - float(config.v9_far_dist)).clamp(min=0.0) * 0.08, torch.zeros_like(score))
            + (float(config.v9_safe_reaction_gap) - reaction_gap).clamp(min=0.0) * 0.65
            + (depletion_ratio - float(config.v9_public_depletion_ratio)).clamp(min=0.0) * send * 0.25
        )
        low_value_hard_public = (
            opening_public_sacrifice
            & (prod_t <= float(config.v9_public_low_prod_max))
            & (reaction_gap <= float(config.v9_public_hard_reaction_gap))
            & shared_zone
            & (depletion_ratio >= 0.78)
        )
        score = torch.where(low_value_hard_public, torch.full_like(score, float("-inf")), score)
        score = score - torch.where(
            opening_public_sacrifice & ~low_value_hard_public,
            public_sacrifice_score,
            torch.zeros_like(score),
        )

        contested_center = (
            (int(config.v9_opening_end) <= int(step))
            & (int(step) < int(config.v9_center_step_end))
            & public_target
            & (center_or_middle | shared_zone)
            & unsafe_reaction
            & cand_valid
        )
        center_penalty = (
            float(config.v9_center_public_penalty)
            + torch.where(shared_zone, close_enemy_count * 1.2, torch.zeros_like(score))
            + (float(config.v9_safe_reaction_gap) - reaction_gap).clamp(min=0.0) * 0.55
            + torch.where(source_public_depletion, send * 0.12, torch.zeros_like(score))
        )
        score = score - torch.where(contested_center, center_penalty, torch.zeros_like(score))

        trailing_low_enemy = (
            bool(trailing)
            & is_enemy_target
            & (prod_t <= float(config.v9_public_low_prod_max))
            & (reaction_gap < float(config.v9_safe_reaction_gap))
            & cand_valid
        )
        score = score - torch.where(
            trailing_low_enemy,
            torch.full_like(score, float(config.v9_trailing_low_enemy_penalty)) + close_enemy_count,
            torch.zeros_like(score),
        )
        ...
        bad_leader_pressure = (
            ...
            & leader_asset
            & (
                (prod_t < float(config.v9_leader_asset_prod_min))
                | (reaction_gap < float(config.v9_leader_hold_reaction_gap))
            )
            & cand_valid
        )
        score = score - torch.where(
            bad_leader_pressure,
            torch.full_like(score, float(config.v9_trailing_low_enemy_penalty)),
            torch.zeros_like(score),
        )
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        trace_path = (
            os.environ.get("ORBIT_V9_TRACE_PATH")
