...
CONFIG_4P = replace(
    ...
    # 2026-06-22 V6-tuned resubmit:
    # Replay evidence from both V6 official runs shows the same 4P failure:
    # non-first games are usually decided by step 50-100 after the bot keeps
    # spending from high-production sources into low/contested targets.  Keep
    # the V6 mechanism, but let it activate slightly earlier and accept a
    # slightly wider safe alternative.  This is intentionally a parameter-only
    # 4P adjustment; 2P/3P presets and the base planner are unchanged.
    v6_gap_step_start=42,
    v6_gap_step_end=180,
    v6_prod_gap_trigger=-5.0,
    v6_prod_rank_trigger=3,
    v6_gap_alt_score_gap=5.9,
    v6_gap_depletion_ratio=0.76,
    v6_gap_safe_reaction_gap=1.4,
    v6_gap_force_bonus=0.085,
)
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        trace_path = os.environ.get("ORBIT_V6_TRACE_PATH") or os.environ.get("ORBIT_V5_TRACE_PATH")
