...
class ProducerLiteConfig:
    ...
    # V5 selected-action safety filter. Unlike the rejected V4 branch, this
    # starts from V2 and only changes the current top candidate when a clearly
    # safer close-score alternative exists.
...
def _apply_v5_selected_action_filter(
    *,
    ...
    """Narrow V5 top-action replacement with trace."""
    global _V5_TRACE_LAST
    ...
    before_reason = "ok"
    if bad_top:
        if bool(low_far_neutral[before_idx].detach().item()):
    ...
    score_gap_ok = score >= (before_score - float(config.v5_alt_score_gap))
    ...
    not_bad_alt = ~(low_far_neutral | low_far_nonleader_enemy | depleted_source)
    ...
    alt_mask = (
        ...
        & score_gap_ok
        & not_bad_alt
        & (prod_gain_ok | reaction_ok | depletion_ok)
    )
    ...
    if bool(alt_mask.any()):
        alt_scores = torch.where(alt_mask, score, torch.full_like(score, float("-inf")))
        after_idx = int(torch.argmax(alt_scores).detach().item())
        adjusted = score.clone()
        adjusted[after_idx] = torch.maximum(
            adjusted[after_idx],
            before_score + float(config.v5_force_bonus),
        )
        adjusted[before_idx] = torch.minimum(
            adjusted[before_idx],
            score[after_idx] - float(config.v5_force_bonus),
        )
    ...
    if bool(config.enable_v5_trace):
        ...
        my_prod = float(prod_by_owner[pid].detach().item()) if 0 <= pid < int(player_count) else 0.0
        leader_prod = float(prod_by_owner[leader].detach().item()) if 0 <= leader < int(player_count) else 0.0
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        trace_path = os.environ.get("ORBIT_V5_TRACE_PATH")
        if trace_path and _V5_TRACE_LAST:
            try:
                trace_dir = os.path.dirname(trace_path)
                if trace_dir:
                    os.makedirs(trace_dir, exist_ok=True)
                with open(trace_path, "a", encoding="utf-8") as handle:
                    handle.write(json.dumps(_V5_TRACE_LAST, sort_keys=True) + "\n")
            except Exception:
                pass
