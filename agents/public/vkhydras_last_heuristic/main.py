...
def _commit_fleet(world, moves, spent, target_locked,
    ...
    if os.environ.get("ORBIT_TRACE"):
        try:
            ...
            tgt = world.planet_by_id.get(int(target_id))
            src_p = world.planet_by_id.get(int(src_id))
            if tgt is not None and src_p is not None:
                proj_owner, proj_ships = effective_garrison_at_arrival(
                    tgt, int(turns), world
                )
                src_ships_before = int(src_p.ships)
                tgt_ships_now = int(tgt.ships)
                tgt_owner_now = int(tgt.owner)
            else:
                proj_owner, proj_ships = -2, -1
                src_ships_before = -1
                tgt_ships_now = -1
                tgt_owner_now = -2
            mode = caller
            with open(os.environ["ORBIT_TRACE"], "a") as fh:
                fh.write(
                    "JSON " + json.dumps({
                        ...
                        "tgt": int(target_id),
                        "tgt_owner_now": tgt_owner_now,
                        "tgt_ships_now": tgt_ships_now,
                        "ships_sent": int(ships),
                        "eta": int(turns),
                        "proj_owner_at_arr": int(proj_owner),
                        "proj_ships_at_arr": int(proj_ships),
                        "mode": str(mode),
                        "format": "2P" if world.is_2p else "4P",
                    }) + "\n"
                )
        except Exception as e:
            try:
                with open(os.environ["ORBIT_TRACE"], "a") as fh:
                    fh.write(f"TRACE_ERR {e}\n")
            except Exception:
                pass
